...
def getModel():
    ...
    print(SE.size())
    model = GMAN(SE, SE_dim=64, TE_dim=opt.his_len + opt.seq_len, timestep_in=opt.his_len, channel_in=opt.channelin, channel_out=opt.channelout, device=device).to(device)
    return model
