...
def main():
    ...
    his_len, seq_len, N_NODE, channelin, channelout = 6,6,47,2,1
    TE_DIM = his_len+seq_len
    SE_path = '../data/Japan_prefectures_embedding.txt'
    area_index = np.arange(N_NODE).tolist()
    SE = loadSE(SE_path, area_index).to(device=device)
    print(SE.size())
    model = GMAN(SE, SE_dim=64, TE_dim=TE_DIM, timestep_in=his_len, channel_in=channelin, channel_out=channelout, device=device).to(device)
    summary(model, [(his_len, N_NODE, channelin), (his_len + seq_len, TE_DIM)], device=device)
    return model
